// loader_workspace.go provides the workspace/skills loader used by the PicoFlare agent.
// Loader loads SKILL.md files from workspace/skills/ for context injection.
...
var workspaceFrontmatterRe = regexp.MustCompile(`(?s)^---\r?\n(.*?)\r?\n---\r?\n*`)

// Loader loads skills from a workspace directory (workspace/skills).
// Used by the PicoFlare-style agent for LLM-defined agent scaling.
type Loader struct {
...
func (l *Loader) loadSkill(path string) (string, bool) {
	...
	return workspaceFrontmatterRe.ReplaceAllString(string(data), ""), true
}
