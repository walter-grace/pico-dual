...
func (cb *ContextBuilder) getIdentity() string {
	...
	// Detect S3 vs filesystem memory backend for accurate self-description
	usesS3 := false
	memoryBucketName := ""
...
Use the list_memory_bucket tool to list bucket contents or count folders/projects.`, bucketLine, workspacePath)
		memoryRule = "3. **Memory** - When remembering something, write to memory/MEMORY.md (the tools will store it in S3)."
	} else {
		identity = `# picoclaw 🦞
...
- Skills: %s/skills/{skill-name}/SKILL.md`, workspacePath, workspacePath, workspacePath, workspacePath)
		memoryRule = fmt.Sprintf("3. **Memory** - When remembering something, write to %s/memory/MEMORY.md", workspacePath)
	}
