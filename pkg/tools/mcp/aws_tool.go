...
// NewAWSMCPClientFromConfig creates an AWS MCP client from PicoClaw config.
...
// AWSMCPToolProxy is a PicoClaw tool that proxies to an AWS MCP server tool.
...
func NewAWSMCPToolProxy(mcpTool *mcp.Tool, client *AWSMCPClient) *AWSMCPToolProxy {
	picoName := awsToolPrefix + mcpTool.Name
...
// Name returns the PicoClaw tool name (aws__<mcp_name>).
